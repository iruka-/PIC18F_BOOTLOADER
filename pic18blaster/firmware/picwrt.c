...
//	ポート出力データ.    
#define	PGC	 		LATBbits.LATB1		// RB1 TCK
#define	PGD	 		LATBbits.LATB0		// RB0 TDO
#define	PGM	 		LATCbits.LATC7		// RC7 TDI
#define	MCLR 		LATBbits.LATB3		// RB3 TMS

//	ポート読み込み.
#define	inPGC	 	PORTBbits.RB1
#define	inPGD	 	PORTBbits.RB0
#define	inPGM	 	PORTCbits.RC7
#define	inMCLR 		PORTBbits.RB3

//	方向レジスタ (0=出力)
#define	dirPGC	 	TRISBbits.TRISB1
#define	dirPGD	 	TRISBbits.TRISB0
#define	dirPGM	 	TRISCbits.TRISC7
#define	dirMCLR 	TRISBbits.TRISB3
...
static void set_tms(int tms)
{
	if(tms) {
		TMS=1;
//		ISP_OUT |=  (1 << TMS);	/* TMS High */
	}else{
		TMS=0;
//		ISP_OUT &= ~(1 << TMS);	/* TMS low */
	}
}
