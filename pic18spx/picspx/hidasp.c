 ...
 * 2010-04-13 : for pic18spx, hidmon, bootmon88
 */
#include <windows.h>
#include <stdio.h>
#include "usbhid.h"
...
//			fprintf(stderr, "Warnning: Please update HIDaspx firmware.\n");
			fprintf(stderr, "Warnning: Please check %s mode.\n", MY_Product);
			once++;
		}
...
int hidasp_list(char * string)
{
	...
	if (found_hidaspx==0) {
		fprintf(stderr, "%s: %s(VID=%04x, PID=%04x) isn't found.\n", string, MY_Product, MY_VID, MY_PID);
	}
	if (hHID_DLL) {
...
#if DEBUG
	fprintf(stderr, "%s Connection check!\n", MY_Produvt);
#endif
