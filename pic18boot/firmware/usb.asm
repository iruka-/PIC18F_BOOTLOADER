...
usb_initialized			res		2		; 初期化済みマーク.
#endif
;-----------------------------------------------------------------------------
...
;		nop
;		nop
		rcall	usb_init_ok		; 6 nop分の時間を別のところで消費してくる.
;; bugfix!! ここまで.
...
;;		btfss	INDF0, DTS		; BDT_STAT(ep0Bi) 元からのバグ?
...
;-----------------------------------------------------------------------------
;###
;void USBCtrlTrfTxService(void)  DEV_TO_HOST (get_descriptorの返却等)
;
...
;-----------------------------------------------------------------------------
;	EP0 Out(SetupPkt) のアドレスと BUFF_SIZE を仮設定する.
;-----------------------------------------------------------------------------
;	戻るときに FSR0は 必ず BDT_STAT(ep0Bo) を指しているので、
;		EP0 の STAT をそのまま設定可能.
SetupPkt_FSR0
